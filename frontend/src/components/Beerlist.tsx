              ...
              <tr key={b.id} className="odd:bg-white even:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-2">
                    {b.wholesalers?.map(w => (
                      <span key={w.wholesalerId} className="inline-flex items-center gap-2 rounded-full border px-2 py-1">
                        <span className="font-medium">{w.name}</span>
                        <span className="text-xs text-gray-500">stock {w.stock}</span>
                        <span className="text-xs">• {w.salePrice.toFixed(2)} €</span>
                      </span>
                    ))}
                    {(!b.wholesalers || b.wholesalers.length === 0) && <span className="text-gray-500">—</span>}
                  </div>
                </td>
              </tr>
            ))}
